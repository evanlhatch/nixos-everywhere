...
# It prepares the environment and executes nixos_everywhere.sh.
...
# Prepare environment variables for nixos_everywhere.sh
# The names must match what nixos_everywhere.sh expects (e.g., FLAKE_URI_INPUT)
...
# --- Get nixos_everywhere.sh ---
NIXOS_EVERYWHERE_SCRIPT_PATH="/tmp/nixos_everywhere.sh"

if [ -n "${NIXOS_EVERYWHERE_EMBEDDED_BASE64}" ]; then
    log_wrapper_info "Decoding embedded nixos_everywhere.sh..."
    echo "${NIXOS_EVERYWHERE_EMBEDDED_BASE64}" | base64 --decode > "${NIXOS_EVERYWHERE_SCRIPT_PATH}"
    if [ ! -s "${NIXOS_EVERYWHERE_SCRIPT_PATH}" ]; then
        log_wrapper_error "Embedded nixos_everywhere.sh is empty after decoding! Cannot proceed."
        exit 1
    fi
    log_wrapper_info "Successfully decoded embedded nixos_everywhere.sh to ${NIXOS_EVERYWHERE_SCRIPT_PATH}"
elif [ -n "${NIXOS_EVERYWHERE_SCRIPT_URL}" ]; then
    log_wrapper_info "Downloading nixos_everywhere.sh from ${NIXOS_EVERYWHERE_SCRIPT_URL}..."
    # Retry curl command
    for i in {1..3}; do
        if curl -L --fail --silent --show-error "${NIXOS_EVERYWHERE_SCRIPT_URL}" -o "${NIXOS_EVERYWHERE_SCRIPT_PATH}"; then
            ...
            log_wrapper_error "Failed to download nixos_everywhere.sh (attempt $i/3). Output/Error above."
            if [ $i -eq 3 ]; then
                log_wrapper_error "Cannot download nixos_everywhere.sh after 3 attempts. Aborting."
                exit 1
            fi
    ...
    if [ ! -s "${NIXOS_EVERYWHERE_SCRIPT_PATH}" ]; then
        log_wrapper_error "Downloaded nixos_everywhere.sh is empty! Cannot proceed."
        exit 1
    fi
    log_wrapper_info "Successfully downloaded nixos_everywhere.sh to ${NIXOS_EVERYWHERE_SCRIPT_PATH}"
else
    log_wrapper_error "Neither embedded script nor URL for nixos_everywhere.sh was provided. Cannot proceed."
    exit 1
fi

chmod +x "${NIXOS_EVERYWHERE_SCRIPT_PATH}"
log_wrapper_info "Made nixos_everywhere.sh executable."

# --- Execute nixos_everywhere.sh ---
log_wrapper_info "Executing ${NIXOS_EVERYWHERE_SCRIPT_PATH}..."
# The nixos_everywhere.sh script itself handles detailed logging to its own file
...
if "${NIXOS_EVERYWHERE_SCRIPT_PATH}"; then
    log_wrapper_info "nixos_everywhere.sh script execution finished successfully."
    # The nixos_everywhere.sh script should handle rebooting.
else
    script_exit_code=$?
    log_wrapper_error "nixos_everywhere.sh script execution FAILED with exit code ${script_exit_code}."
    # Ensure cloud-init knows about the failure.
    exit "${script_exit_code}"
fi

log_wrapper_info "--- nixos_convert_on_debian.sh wrapper finished at $(date) ---"
# If nixos_everywhere.sh handles reboot, this script might not complete its final log message if reboot is immediate.
